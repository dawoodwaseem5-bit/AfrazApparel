...
export default function Services() {
  return (
    <section id="services" className="w-full py-24 bg-white relative border-t border-gray-100">
      <div className="absolute top-0 left-0 w-1/3 h-1/2 bg-brand-50/50 blur-[120px] rounded-full pointer-events-none"></div>
      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, index) => (
            <div 
              key={index} 
              className="bg-white p-8 rounded-2xl shadow-lg shadow-gray-200/50 hover-lift border border-gray-100 group transition-all duration-300 hover:border-accent/40 relative overflow-hidden"
            >
              ...
              <div className="w-16 h-16 rounded-full bg-brand-50 flex items-center justify-center mb-6 border border-brand-100 group-hover:bg-accent/10 group-hover:border-accent/30 transition-colors">
                {service.icon}
              </div>
              <h3 className="text-xl font-bold text-black mb-3 font-playfair">{service.title}</h3>
              <p className="text-sm text-gray-600 leading-relaxed transition-colors">{service.description}</p>
            </div>
          ))}
        </div>

        <div className="mt-20 flex flex-col items-center justify-center p-12 bg-[#1c1c1c] rounded-3xl text-center relative overflow-hidden shadow-2xl border-t-2 border-accent">
          ...
          <div className="relative z-10">
             <h3 className="text-3xl font-playfair font-bold text-white mb-4">Ready to start your production run?</h3>
             <p className="text-gray-300 max-w-xl mx-auto mb-8">
               Partner with AfrazApparel to bring your fashion designs to life with enterprise-level precision.
             </p>
             <a href="#contact" className="px-8 py-4 bg-accent text-white font-bold rounded-full hover:bg-white hover:text-black transition-colors duration-300 shadow-lg shadow-accent/20">
